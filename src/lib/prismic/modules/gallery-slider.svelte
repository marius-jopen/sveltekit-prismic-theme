<script>
	...
	function getCaption(key) {
		if (isFilled.richText(input[key].image_caption_images)) {
			currentCaption = asText(input[key].image_caption_images)
		} else {
			currentCaption = ''
		}
	}
		...
		{#each input as item}
			<SplideSlide>
				<div class="overflow-hidden">
					...
					<!-- <Text field={item.image_caption_images} classes="text-center" /> -->
				</div>
			</SplideSlide>
		{/each}
	</Splide>
