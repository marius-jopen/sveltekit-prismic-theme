	...
	<div transition:fade>
		<div class="box z-0 w-full bg-background left-0 top-0 fixed flex flex-col justify-center ">
			<div class="text-center py-3 px-4 flex flex-col">
				<!-- All items -->
				<a on:click={e => close()} class="text-center py-1 sm:py-0 ml-6 text-lg textHoverGrey" href="/{type}s" sveltekit:prefetch >
					All
				</a>

				<!-- List of filtered items -->
				{#each filteredItems as filterItem}
					<a on:click={e => close()} class="text-center py-1 sm:py-0 ml-6 text-lg textHoverGrey" href="/{type}s/category/{filterItem.replace(" ", '-').toLowerCase()}" sveltekit:prefetch >
						{filterItem}
					</a>
				{/each}
			</div>
